...
final countIncrementer = Provider(
  (ref) {
    return CounterIncrementer(ref.read);
  },
);

class CounterIncrementer {
  CounterIncrementer(this._read);

  final Reader _read;

  void execute() {
    final updateType = _read(selectedUpdateType).state;
    logger.info('updateType: $updateType');
    final doc = _read(myUserDocProvider).data?.value;
    if (doc == null) {
